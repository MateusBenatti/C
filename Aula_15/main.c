...
void insereNodeNoComecoDaLista(struct node**, int);
...
int main(void){
	...
	for (int i = 0; i < 3; i++) {
		...
		insereNodeNoComecoDaLista(&primeiroNodeInicio, x);
...
void insereNodeNoComecoDaLista(struct node** pNode, int numero) {
	...
	if(pNode != NULL){
		...
		novoNode = (struct node*)malloc(sizeof(struct node));
		novoNode->dado = numero;
		novoNode->proximoNode = primeiroNode;
